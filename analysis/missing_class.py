...
def test(model, device, data_loader,  unlearn_class_list, ignore_class_list, class_label_names, num_classes =10, \
    ...
    if ignore_class_list:
        ignore_loss = sum([float(dict_classwise_loss[key]) for key in dict_classwise_loss if key in ignore_class_list])/ sum([float(val) for i, val in enumerate(cm.sum(axis=1)) if i in ignore_class_list])
        ignore_acc = sum([float(val) for i, val in enumerate(cm.diagonal()) if i in ignore_class_list])/ sum([float(val) for i, val in enumerate(cm.sum(axis=1)) if i in ignore_class_list])
        ignore_class_name = [name for i, name in enumerate(class_label_names) if i in ignore_class_list]
    else:
        ignore_acc = 0
        ignore_loss = 0
        ignore_class_name = []
    retain_class_list = unlearn_class_list+ignore_class_list
    retain_acc = sum([float(val) for i, val in enumerate(cm.diagonal()) if i not in retain_class_list])/ sum([float(val)  for i, val in enumerate(cm.sum(axis=1)) if i not in retain_class_list])
    retain_loss = sum([float(dict_classwise_loss[key]) for key in dict_classwise_loss if key not in retain_class_list])/ sum([float(val) for i, val in enumerate(cm.sum(axis=1)) if i not in retain_class_list])
    ...
    if plot_cm:
        ...
        plt.tight_layout()
        plt.savefig(f"./images/cm_{set_name}_{unlearn_class_name}_{job_name}.pdf")
...
def our_unlearn(args, model, device, retain_loader, forget_loader, train_loader, test_loader, train_dataset, val_index = None):
    ...
    for i in range(args.num_classes):
        if i !=  args.unlearn_class[0] and i !=  args.ignore_class[0]:
    ...
    model.eval()
    unlearn_model = model
    group_name = f"{args.group_name}-{args.unlearn_method}-unlearn{args.unlearn_class}"
    
    run = wandb.init(
            # Set the project where this run will be logged
            project=f"Class-{args.dataset}-{args.project_name}-{args.arch}",
            group= group_name,
            name=f"Class-Ignore{args.ignore_class[0]}" ,
            dir = os.environ["LOCAL_HOME"],
            # Track hyperparameters and run metadata
            config= vars(args)
            )
    retain_acc, forget_acc, best_metric = test(model, device, test_loader,args.unlearn_class, args.ignore_class, args.class_label_names, args.num_classes,
    ...
    for alpha_r in [float(val) for val in args.our_alpha_r.split(",")]: 
        for alpha_f in [float(val) for val in args.our_alpha_f.split(",")]: 
            ...
            inference_model.eval()
            retain_acc, forget_acc, metric = test(inference_model, device, test_loader,args.unlearn_class, args.ignore_class, args.class_label_names, args.num_classes,
                job_name = args.unlearn_method, set_name="Test Set") 
...
def main():
    ...
    test(model, device, test_loader,args.unlearn_class,args.ignore_class, args.class_label_names, args.num_classes,
        job_name = args.unlearn_method, set_name="Final Original Result") 
    test(unlearn_model, device, test_loader,args.unlearn_class, args.ignore_class, args.class_label_names, args.num_classes,
